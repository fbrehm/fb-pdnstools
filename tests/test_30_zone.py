...
class TestPdnsZone(FbPdnsToolsTestcase):
    ...
    def test_verify_fqdn(self):
        ...
        zone = PowerDNSZone.init_from_dict(js_zone, appname=self.appname, verbose=self.verbose)
        if self.verbose > 1:
        ...
        for fqdn in invalid_fqdns_type:
            ...
            LOG.debug(
                "Testing returning None on FQDN {f!r} for zone {z!r} ...".format(
                    f=fqdn, z=zone.name
                )
            )
