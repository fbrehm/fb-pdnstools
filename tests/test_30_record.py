...
class TestPdnsRecord(FbPdnsToolsTestcase):
    ...
    def test_pdns_record_gt(self):
        ...
        for test_set in test_matrix:
            with self.assertRaises(TypeError) as cm:
                LOG.debug(
                    "Comparing {r1!r} with {r2!r} ...".format(r1=test_set[0], r2=test_set[1])
                )
    ...
    def test_pdns_recordset_comment(self):
        ...
        LOG.debug(
            "Empty comment.as_dict(minimal=True): {}".format(
        ...
        LOG.debug("Comment.as_dict(minimal=True): {}".format(pp(comment.as_dict(minimal=True))))
